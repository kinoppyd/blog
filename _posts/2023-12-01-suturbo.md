...
このように、TurboStramもHTMLとHTTPの仕組みの上で動作する、非常に強力なDOM書き換えフレームワークです。なんかjQueryっぽいなと感じるのはこの部分ですが、jQueryと違ってサーバーから指令やDOMが飛んでくるので、元のHTMLがセマンティクスを維持したままとっちらかることがなさそうな点がメリットです。
